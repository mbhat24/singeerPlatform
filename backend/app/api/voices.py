...
async def list_voice_profiles(
    ...
    return VoiceProfileList(
        profiles=[VoiceProfileResponse.model_validate(vp) for vp in result.scalars().all()]
    )
...
async def upload_voice_sample(
    ...
    if duration < settings.MIN_VOICE_SAMPLE_SECONDS:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Audio too short. Minimum {settings.MIN_VOICE_SAMPLE_SECONDS}s required, got {duration:.1f}s",
        )
    ...
    from app.worker.tasks import train_voice_model
    train_voice_model.delay(profile_id)
