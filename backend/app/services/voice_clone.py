...
class VoiceCloningService:
    ...
    Pipeline:
    1. Preprocess uploaded voice sample (resample to 16kHz mono)
    2. Extract HuBERT content features + speaker embedding
    3. Fine-tune RVC model on the user's voice
    4. For inference: convert reference audio to target voice
    """
    ...
    def extract_features(self, audio_path: Path) -> tuple[np.ndarray, np.ndarray]:
        """
        Extract HuBERT content features and speaker embedding.

        Returns (hubert_features, speaker_embedding).
        hubert_features: (T, 256) — content representation
        speaker_embedding: (256,) — voice timbre fingerprint
        """
        ...
        try:
            ...
            hubert.eval()
            with torch.no_grad():
                hubert_features = hubert.units(waveform)  # (T, 256)
        except Exception:
    ...
    def train_voice_model(self, voice_profile_id: str, audio_path: str) -> Path:
        """
        Fine-tune a lightweight RVC-style model on the user's voice.

        For production, this would call the full RVC training pipeline.
        Here we implement a simplified version that:
        1. Extracts HuBERT features
        2. Trains a small feature-to-mel decoder
    ...
    def generate_singing(
        ...
        """
        Generate singing audio in the cloned voice.

        If reference_audio_path is provided, converts that audio to the target voice.
        Otherwise, synthesizes from lyrics using a basic TTS + voice conversion pipeline.
    ...
    def _convert_voice(self, source_path: str, model_dir: Path, output_path: Path) -> Path:
        """
        Convert source audio to target voice using the trained model.

        This is the core RVC inference: source audio → HuBERT features →
        speaker-conditioned decoder → target voice audio.
        """
    ...
    def _synthesize_from_lyrics(self, lyrics: str, model_dir: Path, output_path: Path) -> Path:
        """
        Synthesize singing from lyrics when no reference audio is provided.

        Uses a basic approach: generate a simple melodic waveform from text,
        then apply voice conversion.
        """
    ...
    def _feature_to_waveform(
        self, features: torch.Tensor, speaker_emb: torch.Tensor
    ) -> torch.Tensor:
        """
        Convert acoustic features back to waveform using a simple vocoder.

        In production, this would use HiFi-GAN or BigVGAN.
        Here we use a basic Griffin-Lim-inspired approach.
        """
        n_fft = 1024
        hop_length = 256
        ...
        for _ in range(30):
            ...
            waveform = torch.istft(
                stft.unsqueeze(0).transpose(1, 2),
                n_fft=n_fft,
        ...
        waveform = torch.istft(
            (mel_spec * torch.exp(1j * angles)).unsqueeze(0).transpose(1, 2),
            n_fft=n_fft,
            hop_length=hop_length,
            length=n_frames * hop_length,
        )
