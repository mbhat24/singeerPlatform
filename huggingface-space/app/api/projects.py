...
async def list_projects(
    ...
    return ProjectList(projects=[ProjectResponse.model_validate(p) for p in result.scalars().all()])
...
async def create_project(
    ...
    if not voice_profile:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Voice profile not found or not yet trained")
...
async def download_output(
    ...
    result = await db.execute(select(Project).where(Project.id == project_id, Project.user_id == current_user.id, Project.status == ProjectStatus.COMPLETED))
    project = result.scalar_one_or_none()
    if not project or not project.output_audio_path:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Output audio not available")
    ...
    return FileResponse(path, media_type="audio/wav", filename=f"{project.title}.wav")
